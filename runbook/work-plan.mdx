...
| Ubuntu OS upgrade block on all nodes | **~1.5–2 hours** | Maintenance window |
| Powered-off snapshot/checkpoint gate | **~30 minutes** | When required by the current procedure |
...
<Steps>
  ...
  <Step title="Pre-check the DPs and protect recovery">
    Confirm connectivity and current versions, create the required pre-upgrade hypervisor snapshot(s), and pause DP services only when the current generated procedure tells you to.
  </Step>
  <Step title="Upgrade Ubuntu on every required node">
    Follow `16.04 → 18.04 → 20.04 → 22.04 → 24.04` in order. Each hop is a separate controlled transition.
  </Step>
  <Step title="Satisfy any powered-off snapshot gate">
    If the current upgrade client instructs you to shut down and create a powered-off snapshot/checkpoint before continuing, complete that gate on the required nodes before the next stage.
  </Step>
...
| Snapshot/checkpoint gate | Any snapshot action explicitly required by the current client is complete |
